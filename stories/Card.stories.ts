...
const SimpleCardComponent = (args: any, content: any) => {
  return html`
    <vg-card 
...
const InteractiveCardComponent = (args: any, content: any) => {
  ...
  const onaction = (e: CustomEvent) => {
    ...
    setLastEventType(e.detail.originalEvent.type);
  }
  ...
  return html`
    ...
    <div style="margin-top: 12px; font-size: 14px; color: #666;">
      Action Count: <b>${actionCount}</b>
      ${lastEventType ? html`<br />Last Triggered By: <b>${lastEventType}</b>` : ''}
    </div>
  `
}
...
export const Elevated: Story = {
  ...
  render: (args) => SimpleCardComponent(args, html`
...
export const Interactive: Story = {
  ...
  render: (args) => InteractiveCardComponent(args, html`
